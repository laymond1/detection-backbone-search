...
from supernet.supernet import MobileNetV2
from latency_lookup_table import count_conv_flop, MBv2LatencyTable
...
def get_args():
    ...
    parser.add_argument('--cand-mode', type=str, default='flops', help='choose flops or latency to constraint model')
        ...
        if args.cand_mode == 'latency':
            ...
            def get_uniform_sample_cand(*,timeout=500):
                ...
                for i in range(timeout):
                    ...
                    net_config = model.config(cand)
                    if l*1e6 <= lut.predict_network_latency_given_config(net_config) <= r*1e6:
                        return cand
                return get_random_cand()
        elif args.cand_mode == 'flops':
            # for flops
            lut = MBv2LatencyTable(local_dir='.', url='mobile.yaml')
            len_cand = 17 if '300M' in args.model_size else 40
            get_random_cand = lambda:tuple(np.random.randint(3) for i in range(len_cand))
            ...
            def get_uniform_sample_cand(*,timeout=500):
                ...
                for i in range(timeout):
                    cand = get_random_cand()
                    net_config = model.config(cand)
                    if l*1e6 <= lut.count_flops_given_config(net_config) <= r*1e6:
                        return cand
                return get_random_cand()
